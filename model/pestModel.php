<?php
    class pestModel{
        ...
        public function insertar($id, $enunciado, $punto){
            $query = $this->PDO->prepare("INSERT INTO pest(id, enunciado, punto) values(:id, :enunciado, :punto)");
        ...
        public function verForm($id){
            $query = $this->PDO->prepare("SELECT * FROM pest WHERE id = :id");
            $query->bindParam(":id", $id);
            return ($query->execute()) ? $query->fetchAll() : false;    
        }
        public function deleteForm($id){
            $query = $this->PDO->prepare("DELETE FROM pest where id = :id");
            $query->bindParam(":id", $id);
            return $query->execute();
        }
